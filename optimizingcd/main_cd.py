...
def swap(S, qubit_id1, qubit_id2, ps=1):
            ...
            for m, qubit in enumerate(S[node][j]):
                ...
                if not qubit is None:
                    ...
                    if not S[node][j][m][2][0] in vneighborhood:
                        ...
                        vneigh += 1
    return vdeg, vneigh, vneighborhood, vneigh_links
...
def find_pareto_region_cd(users, varying_array, varying_param, protocol, data_type, topology, n, p_gen, q_swap, p_swap, p_cons, cutoff, max_links_swapped, qbits_per_channel, N_samples, total_time, randomseed):
    '''
        ---Inputs---
            · ...
...
def plot_avgs(users, protocol, data_type, topology, n, p_gen, q_swap, p_swap, p_cons, cutoff, max_links_swapped, qbits_per_channel, N_samples, total_time, randomseed, nd_label=None, avg_vdegs_theory=None, avg_vneighs_theory=None, physical_degrees=None, steady_state_window=None, steady_state_force_find=False, dark=False, legend='nodes', save=False, x_cm=8, y_cm=5, fontsize=8, xlimits=None, ylimits_deg=None, ylimits_neigh=None, num_y_ticks_vdeg=4, num_y_ticks_vneigh=4, **kwargs):
    ...
    if True:
        ...
        for idx, node in enumerate(users):
            ...
            stderror_vdegs = 2*std_vdegs_sim_i/np.sqrt(N_samples) #3*min(cutoff,qbits_per_channel)/np.sqrt(N_samples)
...
def plot_pareto_singleprotocol_2users(users, varying_array, varying_param, protocol, data_type, topology, n, p_gen, q_swap, p_swap, p_cons, cutoff, max_links_swapped, qbits_per_channel, N_samples, total_time, randomseed, physical_degrees=None, steady_state_window=None, steady_state_force_find=False, label_x=None, label_y=None, constraint_x=None, constraint_y=None, x_cm=8, y_cm=5, fontsize=8, num_x_ticks=3, num_y_ticks=3, dark=False, save=False, **kwargs):
    '''
        ---Inputs---
            · ...
            · users:    (list of ints) nodes that will be analyzed.
    ...
    if save:
        ...
        filename += 'pareto_vneighs-vary_%s-constrs_%s_%s-%s_%s_n%d-p_gen%.3f-q_swap%.3f'\
                ...
                topology, n, p_gen, q_swap, p_swap, p_cons, cutoff, max_links_swapped,
                qbits_per_channel, N_samples, total_time, randomseed)
        plt.savefig(filename, dpi=300, bbox_inches='tight')
    else:
        plt.show()
...
def plot_avgs_vs_param(users, varying_array, varying_param, protocol, data_type, topology, n, p_gen, q_swap, p_swap, p_cons, cutoff, max_links_swapped, qbits_per_channel, N_samples, total_time, randomseed, dark=False, legend='levels', save=False, steady_state_window=None, steady_state_force_find=False, x_cm=8, y_cm=5, fontsize=8, xlimits=None, ylimits_deg=None, ylimits_neigh=None, num_y_ticks_vdeg=4, num_y_ticks_vneigh=4, optimal_value=True, **kwargs):
...
def plot_colormap(x_array, y_array, data, x_label, y_label, cbar_label, cbar_max=5, cbar_min=0, annotate_min=False, annotate_max=False, annotation_error=0, filename=None):
